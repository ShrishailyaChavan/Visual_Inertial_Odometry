...
from utils import Isometry3d, from_two_vectors, to_rotation, to_quaternion, skew, quaternion_multiplication, quaternion_normalize, small_angle_quaternion
...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        self.sum_angular_vel = np.zeros(3)
        self.sum_linear_acc = np.zeros(3)

        for imu_message in self.imu_msg_buffer:

            self.sum_angular_vel += imu_message.angular_velocity
            self.sum_linear_acc += imu_message.linear_acceleration
        
        self.state_server.imu_state.gyro_bias = self.sum_angular_vel / len(self.imu_msg_buffer)

        # Find the gravity in the IMU frame.
        self.gravity_imu = self.sum_linear_acc / len(self.imu_msg_buffer)
        
        # Normalize the gravity and save to IMUState          
        g_normalization = np.linalg.norm(self.gravity_imu)
        IMUState.gravity = np.array([0.0, 0.0, -g_normalization])

        # Initialize the initial orientation, so that the estimation
        # is consistent with the inertial frame.
        self.state_server.imu_state.orientation = from_two_vectors(-IMUState.gravity, self.gravity_imu)
    ...
    def batch_imu_processing(self, time_bound):
        ...
        self.used_imu_msg_center = 0
        for imu_message in self.imu_msg_buffer:
            imu_time = imu_message.timestamp

            if imu_time < self.state_server.imu_state.timestamp:
                self.used_imu_msg_center += 1
                continue
            if imu_time > time_bound:
                break

            self.process_model(imu_time, imu_message.angular_velocity, imu_message.linear_acceleration)
            self.used_imu_msg_center += 1

            # Update the state info
            self.state_server.imu_state.timestamp = imu_time ## This was not there before
        ...
        self.imu_msg_buffer = self.imu_msg_buffer[self.used_imu_msg_center:]
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        # self.imu_state.acc_bias = np.zeros(3)

        self.imu_state = self.state_server.imu_state
        gyro = m_gyro - self.imu_state.gyro_bias
        acc = m_acc - self.imu_state.acc_bias
        dtime = time - self.imu_state.timestamp

        # Compute discrete transition F, Q matrices in Appendix A in "MSCKF" paper
        F = np.zeros((21,21))
        G = np.zeros((21,12))

        F[0:3, 0:3] = -skew(gyro) ## (3,3) 0,0
        F[0:3, 3:6] = -np.identity(3) ## (3,3) 0,3
        F[6:9, 0:3] = np.matmul(-np.transpose(to_rotation(self.imu_state.orientation)),skew(acc)) ##(3,3) 6,0
        F[6:9, 9:12] = -np.transpose(to_rotation(self.imu_state.orientation))
        F[12:15, 6:9] = np.identity(3) ##(3,3) 12,6

        G[0:3, 0:3] = -np.identity(3) ##(3,3) 0,0
        G[3:6, 3:6] = np.identity(3) ##(3,3) 3,3
        G[6:9, 6:9] = -np.transpose(to_rotation(self.imu_state.orientation)) ##(3,3) 6,6
        G[9:12, 9:12] = np.identity(3) ##(3,3) 9,9
        
        # Approximate matrix exponential to the 3rd order, which can be 
        # considered to be accurate enough assuming dt is within 0.01s.
    
        Fdt = F * dtime
        Fdt_square = np.matmul(Fdt,Fdt)
        Fdt_cube = np.matmul(Fdt_square,Fdt)
        ...
        R_kk_1 = to_rotation(self.imu_state.orientation_null)
        R_kk_1_t = np.transpose(R_kk_1)
        Phi[0:3, 0:3] = np.matmul(to_rotation(self.imu_state.orientation),R_kk_1_t)

        u = np.matmul(R_kk_1,IMUState.gravity)
        s = u / np.matmul(u, u)

        A1 = Phi[6:9, 0:3]
        w1 = np.matmul(skew(self.imu_state.velocity_null-self.imu_state.velocity),IMUState.gravity)
        Phi[6:9, 0:3] = A1 - (np.matmul(A1,u)-w1)[:, None] * s  ## A bit different

        A2 = Phi[12:15, 0:3]
        w2 = np.matmul(skew(self.imu_state.velocity_null + self.imu_state.position_null - self.imu_state.position),IMUState.gravity)
        Phi[12:15, 0:3] = A2 - (np.matmul(A2,u)-w2)[:, None]*s  ## A bit different

        # Propogate the state covariance matrix.
        Q = Phi @ G @ self.state_server.continuous_noise_cov @ G.T @ Phi.T * dtime  ## Change this!!
        ...
        if len(self.state_server.cam_states) > 0:
            ...
            self.state_server.state_cov[21:, 0:21] = self.state_server.state_cov[21:, 0:21]  @ np.transpose(Phi)
        

        # Fix the covariance to be symmetric
        state_cov_fixed = (self.state_server.state_cov + np.transpose(self.state_server.state_cov)) / 2.0
        self.state_server.state_cov = state_cov_fixed

        # Update the state correspondes to null space
        self.imu_state.orientation_null = self.imu_state.orientation
        self.imu_state.position_null = self.imu_state.position
        self.imu_state.velocity_null = self.imu_state.velocity
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        Omega[0:3, 3] = gyro     ## This was wrong before. Now changed (3:4 is wrong, only 3 is correct)
        Omega[3, 0:3] = -gyro
        ...
        if gyro_norm > 1e-5 :
            dq_dt = (np.cos(gyro_norm*dt*0.5)*np.identity(4) + (1/gyro_norm)*np.sin(gyro_norm*dt*0.5) * Omega) @ q
            dq_dt2 = (np.cos(gyro_norm*dt*0.25)*np.identity(4) + 1/gyro_norm*np.sin(gyro_norm*dt*0.25) * Omega) @ q
        else :
            dq_dt = (np.identity(4)+0.5*dt*Omega) * np.cos(gyro_norm*dt*0.5) @ q
            dq_dt2 = (np.identity(4)+0.25*dt*Omega) * np.cos(gyro_norm*dt*0.25) @ q
        ...
        # k1 = f(tn, yn)
        q_t = np.transpose(to_rotation(q))     
        k1_v_dot = np.matmul(q_t,acc) + IMUState.gravity  
        k1_p_dot = v
        

        # k2 = f(tn+dt/2, yn+k1*dt/2)
        k1_v = v + k1_v_dot*dt/2
        k2_v_dot = np.matmul(dR_dt2_transpose, acc) + IMUState.gravity
        k2_p_dot = k1_v
        
        # k3 = f(tn+dt/2, yn+k2*dt/2)
        k2_v = v + k2_v_dot*dt/2
        k3_v_dot = np.matmul(dR_dt2_transpose, acc) +IMUState.gravity
        k3_p_dot = k2_v
        
        # k4 = f(tn+dt, yn+k3*dt)
        k3_v = v + k3_v_dot*dt; 
        k4_v_dot = np.matmul(dR_dt_transpose,acc) + IMUState.gravity
        k4_p_dot = k3_v
    ...
    def state_augmentation(self, time):
        ...
        R_w_c = np.matmul(R_i_c, R_w_i)
        R_w_i_t = np.transpose(R_w_i)
        t_c_w = self.state_server.imu_state.position + np.matmul(R_w_i_t,t_c_i)

        self.cam_state = CAMState(self.state_server.imu_state.id)

        self.cam_state.timestamp = time
        self.cam_state.orientation = to_quaternion(R_w_c)
        self.cam_state.position = t_c_w

        self.cam_state.orientation_null = self.cam_state.orientation
        self.cam_state.position_null = self.cam_state.position

        self.state_server.cam_states[self.state_server.imu_state.id] = self.cam_state
        ...
        J[3:6, 0:3] = skew(np.matmul(R_w_i_t,t_c_i)) 
        J[3:6, 12:15] = np.identity(3) 
        J[3:6, 18:21] = np.transpose(R_w_i)   

        # Resize the state covariance matrix.
        
        previous_values = self.state_server.state_cov.shape[0]   
        state_cov = np.zeros((previous_values+6, previous_values+6))
        state_cov[:previous_values, :previous_values] = self.state_server.state_cov

        # Fill in the augmented state covariance. 
        state_cov[previous_values:, :previous_values] = J @ state_cov[:21, :previous_values]
        state_cov[:previous_values, previous_values:] = state_cov[previous_values:, :previous_values].T
        state_cov[previous_values:, previous_values:] = J @ state_cov[:21, :21] @ np.transpose(J)

        # Fix the covariance to be symmetric
        state_cov_fixed = (state_cov + np.transpose(state_cov)) / 2.0
        self.state_server.state_cov = state_cov_fixed
    ...
    def measurement_update(self, H, r): 
        ...
        if H.shape[0] > H.shape[1]:
            # QR decomposition
            # H_sparse = sparse.csr_matrix(H) ## This is not there in uoip
        ...
        K_transpose = scipy.linalg.solve(S, np.matmul(H_thin,P))  ## Remove ldl if not working
        K = K_transpose.T

        # Compute the error of the state.
        delta_x = np.matmul(K,r_thin)
        
        # Update the IMU state.
        delta_x_imu = delta_x[0:21]

        if np.linalg.norm(delta_x_imu[6:9]) > 0.5 or np.linalg.norm(delta_x_imu[12:15]) > 1.0 :   # 3 6, 3 12
            print("delta velocity: %f\n", np.linalg.norm(delta_x_imu[6:9]))
            print("delta position: %f\n", np.linalg.norm(delta_x_imu[12:15]))
        ...
        self.state_server.imu_state.gyro_bias += delta_x_imu[3:6]  # 3 3
        self.state_server.imu_state.velocity += delta_x_imu[6:9]   #3 6
        self.state_server.imu_state.acc_bias += delta_x_imu[9:12]  # 3 9
        self.state_server.imu_state.position += delta_x_imu[12:15]  # 3 12

        dq_extrinsic = small_angle_quaternion(delta_x_imu[15:18])  # 3 15
        self.state_server.imu_state.R_imu_cam0 = np.matmul(to_rotation(dq_extrinsic),self.state_server.imu_state.R_imu_cam0)
        self.state_server.imu_state.t_cam0_imu += delta_x_imu[18:21]  # 3 18

        for i, (_, self.cam_state) in enumerate(self.state_server.cam_states.items()):
            delta_x_cam = delta_x[21+i*6:27+i*6]
            dq_cam = small_angle_quaternion(delta_x_cam[0:3])
            self.cam_state.orientation = quaternion_multiplication(dq_cam, self.cam_state.orientation)
            self.cam_state.position += delta_x_cam[3:] 

        # Update state covariance.
        I_KH = np.identity(K.shape[0]) - np.matmul(K,H_thin)
        self.state_cov = np.matmul(I_KH,self.state_server.state_cov)

        # Fix the covariance to be symmetric
        self.state_cov_fixed = (self.state_cov + np.transpose(self.state_cov)) / 2.0
        self.state_server.state_cov = self.state_cov_fixed
    ...
    def publish(self, time):
        ...
        # a = "{:.2f}".format(imu_state.timestamp)
        # b = "{:.2f}".format(imu_state.orientation)
        # c = "{:.2f}".format(imu_state.position)

        f = open('test_f.txt', 'a')
        
        f.write(f"{imu_state.timestamp} {imu_state.position[0]} {imu_state.position[1]} {imu_state.position[2]} {imu_state.orientation[0]} {imu_state.orientation[1]} {imu_state.orientation[2]} {imu_state.orientation[3]}\n")
